...
namespace PhyGen.API.Controllers
{
    ...
    public class ContentFlowController : BaseController<ContentFlowController>
    {
        ...
        public async Task<IActionResult> GetAllContentFlows()
        {
            var request = new GetAllContentFlowsQuery();
            return await ExecuteAsync<GetAllContentFlowsQuery, List<ContentFlowResponse>>(request);
        }

        [HttpGet("{contentFlowId}")]
        [ProducesResponseType(typeof(ApiResponse<ContentFlowResponse>), (int)HttpStatusCode.OK)]
        public async Task<IActionResult> GetContentFlowById(int contentFlowId)
        {
        ...
        public async Task<IActionResult> CreateContentFlow([FromBody] CreateContentFlowRequest request)
        {
            ...
            return await ExecuteAsync<CreateContentFlowCommand, int>(command);
        }
